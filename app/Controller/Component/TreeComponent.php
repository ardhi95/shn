<?php
	...
	class TreeComponent extends Component
	{
		...
		public function buatTree($data_latih, $target_atribut, $arrayatribut = array()) { 
			$kolom  = $target_atribut;
			$kelas = getKelas($data_latih);

			if (count($kelas) === 1) {
				return new TreeLabel($kelas[0]);
			} elseif (count($arrayatribut) === 0) {
				$nilaidata 	= getNilaidata($data_latih, $kolom);
				$prob   	= hitungProb($data_latih, $kolom);
				$node   	= array(); 

				foreach ($nilaidata as $value) {
					end($prob[$value]); 
					$label        = key($prob[$value]);	
					$node[$kolom." ".$value] = new TreeLabel($label);
				}
				...
				$nilaidata = getNilaidata($data_latih, $kolom);
					$node   = array(); 
					foreach ($nilaidata as $value)	{
						$node[$kolom." ".$value] = buatTree(
							getIsidataDari($data_latih, $kolom, $value), 
							array_pop($arrayatribut),
							$arrayatribut
						);
					}
					return new TreeNode($node);
