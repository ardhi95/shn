    ...
    <div class="dataTables_wrapper no-footer panel-heading">
    	<div style="display:block; width:33.33%;float:left; text-align:left;">
            <div class="dataTables_length" style="border:0px;width:100%;">
                <!--label>
                    Action :
                    <select name="customers2_length" aria-controls="customers2" class="form-control" id="action" style="width:150px;">
                        <option value="">Select Action</option>
                        <option value="delete">Delete</option>
                    </select>
                </label>
                <a href="javascript:void(0);" class="btn btn-primary btn-sm" style="margin-top:-5px;" onclick="ActionChecked()">GO</a-->
            ...
            <table class="table table-bordered table-striped table-actions">
                <thead>
                    <tr>
                        ...
                        <th width="100">
                        	<?php echo $this->Paginator->sort("$ModelName.alias",__('Name'),array("class"=>"sorting"));?>
                        </th>
                        <th width="100">
                        	<?php echo $this->Paginator->sort("Parent.alias",__('Parent'),array("class"=>"sorting"));?>
                        </th>
                        <th width="50">
                ...
                <tbody>
                    ...
                    <?php
						...
						{
							$parent	=	($data["Parent"]['id'] == null) ? "-" : $data["Parent"]['alias'];
						}
						else
						{
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <a href="<?php echo $settings['cms_url'].$ControllerName?>/Edit/<?php echo $data[$ModelName]["id"]?>/<?php echo $page?>/<?php echo $viewpage?>" class="btn btn-info btn-condensed btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo __('Edit')?>"><i class="fa fa-pencil"></i></a>


                            <?php if(in_array($data[$ModelName]['id'],$child)):?>
                            	<a href="javascript:void(0);" class="btn btn-danger btn-condensed btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo __('Delete')?>" onclick="Delete(this,'<?php echo __('Do you realy want to delete this item ?')?>','<?php echo $data[$ModelName]['id']?>')"><i class="fa fa-times"></i></a>
                            <?php else:?>
                            	<a href="javascript:void(0);" class="btn btn-danger btn-condensed btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo __('Delete')?>" disabled="disabled"><i class="fa fa-times"></i></a>
                            <?php endif?>


							<?php if($login_id == $super_admin_id):?>
                            	<a href="<?php echo $settings["cms_url"].$ControllerName?>/SettingPrivileges/<?php echo $data[$ModelName]["id"]."/".$page."/".$viewpage?>" class="btn btn-primary btn-condensed btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo __('Setting Privileges')?>"><i class="fa fa-gear"></i></a>
                            <?php else:?>
								<?php if(in_array($data[$ModelName]['id'],$child)):?>
                                <a href="<?php echo $settings["cms_url"].$ControllerName?>/SettingPrivileges/<?php echo $data[$ModelName]["id"]."/".$page."/".$viewpage?>" class="btn btn-primary btn-condensed btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo __('Setting Privileges')?>"><i class="fa fa-gear"></i></a>
                                <?php else:?>
                                <a href="javascript:vod(0)"  class="btn btn-primary btn-condensed btn-sm" data-toggle="tooltip" data-placement="top" title="<?php echo __('Setting Privileges')?>" disabled="disabled"><i class="fa fa-gear"></i></a>
                                <?php endif?>
                            <?php endif;?>
                        </td>
                    </tr>
