 ...
 */
class TreeCmsHelper extends AppHelper {
/**
 * name property
 *
 * @var string 'Tree'
 ...
 * @access public
 */
    function generate ($data, $settings = array (),$current_selected=array()) {
        ...
        foreach ($data as $i => $result) {
            ...
            if ($element) {
				...
				$checked	=	(in_array($result[$model]["id"],$current_selected)) ? "checked='checked'" : "";
				if($hasChildren==true)
				{
            		$content = '<input type="checkbox" name="data['.$name.'][]['.$key_name.']" value="'.$result[$model]["id"].'" '.$checked.'/><span class="folder">'.$result[$model][$alias].'</span>';

            	}
            	else
            	{
            		$content = '<input type="checkbox" name="data['.$name.'][]['.$key_name.']" value="'.$result[$model]["id"].'" '.$checked.'/><span class="file">'.$result[$model][$alias].'</span>';
            	}
            ...
            if ($__addType) {
                ...
                if ($type) {
                    ...
                    if($count==1)
                    {
                    	$return .= '<' . $type .  $typeAttributes . ' class="filetree" id="browser" >';
                    }
                    else
                    {
            ...
            if ($itemType) {
                ...
                if($firstChild)
                {
                	if($count>1 && $admin_review==0)
                    {
						$return .= '<li><span class="add_category" onclick="location.href=\''.Configure::read('webUrl').'Product/AddCategories/'.$result[$model]['parent_id'].'\'">Add Sub Category</span></li><' . $itemType . $itemAttributes . '>';
					}
					else
					{
