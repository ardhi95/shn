...
var $start; 
    function generate ($data, $settings = array (),$aco_id=NULL) {
		...
		$this->start	=	'<li class="xn-logo">
                <a href="'.$this->__settings['cms_url'].'">DINO</a>
                <a href="javascript:void(0);" class="x-navigation-control"></a>
            </li>
            <li class="xn-profile">
                <a href="#" class="profile-mini">
                    <img src="'.$this->__settings['avatar'].'" alt="'.$this->__settings['admin_name'].'" id="avatar-left-mini"/>
                </a>
                <div class="profile">
                    <div class="profile-image">
                        <a href="'.$this->__settings['cms_url'].'Admins/Edit/'.$this->__settings['admin_id'].'" title="'.$this->__settings['admin_name'].'">
							<img src="'.$this->__settings['avatar'].'" alt="'.$this->__settings['admin_name'].'" id="avatar-left-big"/>
						</a>
                    </div>
                    <div class="profile-data">
                        <div class="profile-data-name"> <a href="'.$this->__settings['cms_url'].'Admins/Edit/'.$this->__settings['admin_id'].'" title="'.$this->__settings['admin_name'].'" style="color:#ffffff;">'.$this->__settings['admin_name'].'</a></div>
                        <div class="profile-data-title">'.$this->__settings['admin_group'].'</div>
                    </div>
                </div>                                                                        
            </li>';
        ...
        foreach ($data as $i => $result) {
            ...
            if ($element) {
				...
				if($hasChildren==true)
				{
            		$content = '<a href="javascript:void(0);"><span class="'.$result[$model]['icon_class'].'"></span><span class="xn-text">'.$result[$model][$alias].'</span></a>';
					
            	}
            	else
            	{
            		if($result[$model]['is_group_separator']=="1")
					{
						$content = $result[$model][$alias];
					}
					else
					{
						$content = '<a href="'.$cms_url.$result["MyAco"]["controller"].'/"><span class="'.$result[$model]['icon_class'].'"></span><span class="xn-text">'.$result[$model][$alias].'</span></a>';
					}
            	}
            ...
            if ($__addType) {
                ...
                if ($type) {
                    ...
                    if($count==1)
                    {
                    	$return .= '<' . $type .  $typeAttributes . ' class="x-navigation">'.$this->start;
                    }
                    else
                    {
            ...
            if ($itemType) {
                ...
                if($hasChildren)
				{
					if($result[$model]["aco_id"] == $aco_id)
					{
						$return .= '<' . $itemType . $itemAttributes . ' class="active">';
					}
					else
					{
						$return .= '<' . $itemType . $itemAttributes . ' class="xn-openable">';
					}
				}
				else
				{
					if($result[$model]['is_group_separator']=="1")
					{
						$return .= '<' . $itemType . $itemAttributes . ' class="xn-title">';
					}
					else
					{
						if($result[$model]["aco_id"] == $aco_id)
