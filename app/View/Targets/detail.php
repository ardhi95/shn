...
<script>
	...
	Morris.Line({
		...
		{
			y:'<?php echo $key[0]['modified']; ?>',
			a:<?php echo $key[0]['Pendapatan']; ?>
		},
		<?php endforeach ?>
		],
		xkey: 'y',
    ...
    if($("#reportrange").length > 0){   
        $("#reportrange").daterangepicker({                    
            ...
            endDate: moment()            
                ...
                /*$.getJSON("<?php echo $settings["cms_url"]?>Api/SelesChartData?token=e56d867b3506917898f348720130b55d&firstdate="+stdate+"&enddate="+endate+"&user_id=<?php echo $ID; ?>",
                    function(json){
                   $.each(json.data,function(key, value){
                        data = value[0].modified;
...
<script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCV4QFXiixx190iTfXWeDPSuvnEaHHxsE8&callback=myMap"></script>
                ...
                <div class="panel-body">
                    <div class="table-responsive">
                        <table class="table table-striped" id="example">
                            ...
                            <tbody>
                                <?php $no = 0; ?>
                                <?php foreach ($dataChart as $keyChart): ?>
                                <?php $no++; ?>
                                <tr>
                                    <td><?php echo $no; ?></td>
                                    <td><?php echo $keyChart['orders']['id']; ?></td>
                                    <td><?php echo $keyChart['stores']['name']; ?></td>
                                    <td><?php echo $keyChart['stores']['address']; ?></td>
                                    <td><?php echo $keyChart[0]['modified']; ?></td>
                                    <td>
                                        <?php  
                                            $angka = $keyChart[0]['Pendapatan'];
                                            echo "Rp. ".number_format($angka,0,',','.');
                                        ?>
                                    </td>
                                </tr>
