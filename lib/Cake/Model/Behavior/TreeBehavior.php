 ...
 */
	...
	public function generateTreeList2(Model $Model, $conditions = null, $keyPath = null, $valuePath = null, $spacer = '_', $recursive = null) {
		$overrideRecursive = $recursive;
		extract($this->settings[$Model->alias]);
		if ($overrideRecursive !== null) {
			$recursive = $overrideRecursive;
		...
		return $this->formatTreeList2($Model, $results, compact('keyPath', 'valuePath', 'spacer'));
	}
	
	public function formatTreeList2(Model $Model, array $results, array $options = array()) {
		if (empty($results)) {
			return array();
		}
		$defaults = array(
